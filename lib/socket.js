...
function socket(io, ascoltatore) {
  io.on('connection', function(err, socket, session) {
	...
	_key = ['~user', user].join('+');
      ...
      if (_.has(things, _key)) {
	fn({
	  message: 'you\'re already subscribed to that',
	  code: 304
	})
	return;
      }
      ...
      if (key === 'master') {
	subscribe(things, ['~user', user].join('+'), function(message, unsubscribe) {
	...
	user, key, function(err, owned) {
	  ...
	  subscribe(things, key, function(message, unsubscribe) {
	    if (!socket.disconnected) {
	      socket.emit('message', message);
	      return;
	    }
	...
	_key = ['~user', user].join('+');
      ...
      if (!_.has(things, _key)) {
	fn({
	  message: 'you\'re not subscribed to that',
	  code: 304
	})
	return;
      }
      
      unsubscribe(things, _key, function() {
	fn(null)
      });
    });
  ...
  function subscribe(things, topic, fn) {
    var unsubscribe = function(done) {
      done = done || function() {};
      ascoltatore.unsubscribe(topic, fn, done);
    };
    
    things[topic] = unsubscribe;
    ascoltatore.subscribe(topic, function(topic, packet) {
      fn(packet, unsubscribe);
    });
  }
  
  function unsubscribe(things, topic, fn) {
    things[topic](fn);
  }
